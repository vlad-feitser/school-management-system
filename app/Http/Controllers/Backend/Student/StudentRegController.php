...
class StudentRegController extends Controller
{
    ...
    public function StudentRegStore(Request $request)
    {
        DB::transaction(function () use($request){
            ...
            if($student == null) {
                ...
                if($studentId < 10) {
                    $id_no = '000'.$studentId;
                } elseif($studentId < 100 && $studentId >= 10) {
                    $id_no = '00'.$studentId;
                } elseif($studentId < 1000 && $studentId >= 100) {
                    $id_no = '0'.$studentId;
                }
            } else {
    ...
    public function StudentUpdatePromotion(Request $request,$student_id)
    {
        DB::transaction(function () use($request,$student_id){
            ...
            $assign_student = AssignStudent::where('id',$request->id)->where('student_id', $request->student_id)->first();
